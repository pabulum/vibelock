 ...
 * and the footer. Closes on backdrop click or Escape (native <dialog> via ModalShell). */
export function GuideModal({ onClose }: { onClose: () => void }) {
  return (
    <ModalShell
      label="Methodology and glossary"
    ...
    >
      <section>
        ...
        <p>
          A <strong>young patch</strong> has too few games to judge every item
          on its own, so the build is{" "}
          <strong>backfilled from the 30 days before the patch</strong>: each
          item&rsquo;s pre-patch record counts as prior evidence worth at most
          ~a thousand games, and it fades out automatically as the new patch
          accumulates data. Items whose fresh numbers clearly disagree with
          their pre-patch record (the things the patch actually changed) borrow
          far less, and brand-new items are judged on fresh data alone. The meta
          line shows what share of the evidence is backfilled, and the{" "}
          <strong>Backfill</strong> toggle in the header turns it off — you get
          the selected window raw, thin as it may be.
        </p>
      </section>
      ...
      <section>
        ...
        <p>
          ...
          <em>net worth at the moment of purchase</em>, so an item doesn’t look
          strong just because the team that was already winning happened to buy
          it. Build and item rows use adjusted rates, and show the{" "}
          <strong>± gap versus the hero’s average</strong>.
        </p>
        <p>
      ...
      <section>
        ...
        <ul>
          <li>
            <span className="statetag comeback">comeback</span> adjusted ≫ raw —
            it holds up even when bought from behind. A safer pick when you’re
            losing.
          </li>
          <li>
            <span className="statetag winmore">win more</span> raw ≫ adjusted —
            its win rate leans on already being ahead. Strong when snowballing,
            thin when the game is even.
          </li>
        </ul>
        <p className="fine">
      ...
      <section>
        ...
        <ul>
          <li>
            <strong>
              A <span className="role role-universal">CORE</span> seat can be
              earned by popularity alone.
            </strong>{" "}
          ...
          <li>
            ...
            Each win rate is dragged toward the hero average by how little data
            backs it — a pick with thousands of games barely moves, one with a
            few dozen is pulled most of the way. A shiny niche item won’t
            outrank a proven staple on noise.
          </li>
          <li>
            <strong>Picks are ranked cautiously.</strong> Discretionary slots
            are ordered by the win rate we’re fairly sure a pick{" "}
            <em>at least</em> reaches, so something we’re confident about beats
      ...
      <section>
        ...
        <p>
          ...
          rise and carry the enemy’s portrait — the number is that pick’s raw
          win-rate gain into that hero — while picks that are weak into the comp
          get a <span className="weakcomp">▼</span> flag. Staples and
          per-category soul balance are preserved.
        </p>
        <p>
          A counter is only marked when it beats the matchup&rsquo;s expected
          shift <em>for its own buy timing</em> — some enemies spike early and
          fall off (an item bought at 35 minutes doesn&rsquo;t get credit for
          the enemy&rsquo;s late-game fade), and the edge must be large enough
          to be real at that sample. Counter numbers are raw, so they read
      ...
      <section>
        ...
        <dl className="glossary">
          ...
          <dt>Hero avg</dt>
          <dd>
            The population’s baseline win rate for this hero, rank and patch.
            Rows show ± versus it.
          </dd>

          <dt>pulled to average</dt>
          <dd>
            Small-sample win rates are dragged toward the hero average by how
            little data backs them, so a lucky streak on a rarely-built item
            can’t top the list. The fewer the games, the harder the pull.
          </dd>
          ...
          <dt>significant</dt>
          <dd>
            An edge big enough to be unlikely from chance at that sample size.
            It’s the bar a pick clears to be called a value pick or a counter —
            not just beating a fixed number.
          </dd>

          <dt>synergy</dt>
          <dd>
            Two items that win <em>together</em> more (or less) than their solo
            rates predict. The build favours pairs that reinforce each other and
            avoids redundant ones; it’s baked into which picks fill the
            discretionary slots, not shown as its own list.
          </dd>
          ...
          <dd>
            Adjusted ≫ raw — the pick holds up even when you buy it from behind.
          </dd>
          ...
          <dd>
            Raw ≫ adjusted — the pick’s win rate leans on already being ahead.
          </dd>
          ...
          <dd>
            Seated by pick rate (~30%+ of players build it every game), not by
            win rate — at that popularity the win rate can’t move far either
            way, so it isn’t judged on that number unless buyers are shown to be
            significantly and meaningfully behind everyone who skipped it. A
            small red delta here usually isn’t a red flag.
          </dd>
          ...
          <dd>
            ...
            situational ones. “core N/M” counts our core picks a community build
            also runs; “flex N/M” counts our situational picks it also flags
