  ...
  return createPortal(
    <div className="guide-backdrop" onClick={onClose}>
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label="The Lab — experimental statistics"
      ...
      >
        ...
        <div className="guide-body">
          <p className="labintro">
            The rest of Vibelock works from the live analytics API. This page
            works from a nightly snapshot of <strong>whole matches</strong> —
            every purchase in every game, with the souls scoreboard as it stood
            at that moment. That lets us ask a question the win-rate columns
            can&rsquo;t:{" "}
            <em>
              given how the game was going, did this win more than it should
              have?
            </em>
          </p>
          ...
          {stats && (
            <>
              <section>
                ...
                <p>
                  Take every game a hero played and compare two numbers: how
              ...
              <section>
                ...
                <p>
                  An item&rsquo;s raw win rate mixes two things: what the item
                  does, and{" "}
                  <strong>who was already winning when it got bought</strong>.
                  For every purchase we log the win probability implied by the
                  souls scoreboard at that exact moment (&ldquo;bought at&rdquo;
                  below); <strong>vs expectation</strong> is how the
                  buyers&rsquo; games actually ended, minus that. It reads like
                  the build page&rsquo;s win-rate deltas: positive beats the
                  situations it was bought in, negative is flattered by them.
                  Note <strong>Metal Skin</strong>: bought at{" "}
                  {Math.round(
                    (stats.items.find((i) => i.name === "Metal Skin")?.wpBuy ??
                      0.43) * 100,
                  )}
                  % — the most &ldquo;we&rsquo;re in trouble&rdquo; purchase in
                  the game — which is exactly why its raw win rate always looks
                  terrible.
                </p>
                <p>
                  These numbers are <strong>roster-wide</strong>. Hero-specific
                  versions aren&rsquo;t shown because they don&rsquo;t survive
                  the sample sizes yet — for per-hero item advice, the build
              ...
              <p className="labmeta">
                ...
                nightly. Items shown need 2,000+ purchases; a value under about
                ±1 point is within noise.
                {stats.readiness && (
                  <>
                    {" "}
                    Hero-specific readiness:{" "}
                    {stats.readiness.cellsPastK.toLocaleString()} of{" "}
                    {stats.readiness.cellsTracked.toLocaleString()} hero-item
                    pairs have enough data to stand on their own (median{" "}
