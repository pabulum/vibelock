...
test("the death map plots real positions and names no places", async () => {
  ...
  const dots = must(map).querySelectorAll(".dmdot");
  expect(dots).toHaveLength(9);

  // The population underlay decoded from the baked grid (base64 → bytes → cells).
  expect(must(map).querySelectorAll(".dmdensity rect").length).toBeGreaterThan(
    50,
  );

  // Killer positions render as direction leaders.
  expect(must(map).querySelectorAll(".dmfrom").length).toBeGreaterThan(0);

  // This seat's deaths concentrate, so the repeat-spot ring and its read must both appear.
  expect(must(map).querySelector(".dmcluster")).toBeTruthy();
  const insight = must(map).querySelector(".dminsight")?.textContent ?? "";
  ...
  expect(must(map).querySelectorAll(".dmlm")).toHaveLength(14);
  expect(must(map).querySelector(".dmmid")).toBeTruthy();
  // The map's silhouette: three zipline routes, drawn in our own ink from baked world coordinates
  // rather than as game art.
  expect(must(map).querySelectorAll(".dmzip path")).toHaveLength(3);
  expect(must(map).textContent ?? "").toMatch(/YOUR HALF/);
  expect(must(map).textContent ?? "").toMatch(/THEIR HALF/);
  ...
  expect(must(map).textContent ?? "").not.toMatch(
...
test("the phase filter re-slices the map", async () => {
  ...
  const map = must(document.querySelector(".deathmap"));
  ...
  const lane = must(
    [...map.querySelectorAll<HTMLButtonElement>(".dmfilter button")].find((b) =>
      b.textContent?.startsWith("Lane"),
    ),
  );
  await userEvent.click(lane);
  const laneDots = must(document.querySelector(".deathmap")).querySelectorAll(
    ".dmdot",
  ).length;
