...
describe("encodeUrlState", () => {
  it('omits defaults: the "all" build and empty enemies leave no trace', () => {
    expect(
      encodeUrlState({ hero: "paradox", tier: 11, build: "all", enemies: [] }),
    ).toBe("?hero=paradox&rank=11");
  });
...
describe("decodeUrlState", () => {
  it("parses a full query string (leading ? optional)", () => {
    expect(
      decodeUrlState(
        "?hero=abrams&rank=8&patch=1700000000&build=spirit&vs=seven,haze",
      ),
    ).toEqual({
      hero: "abrams",
      tier: 8,
  ...
  it("drops empty enemy entries", () => {
    expect(decodeUrlState("?vs=seven,,haze,")).toEqual({
      enemies: ["seven", "haze"],
    });
  });
