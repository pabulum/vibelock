...
// build whose item set most *aligns* with ours. When they're the same build, that's a
 ...
 * matches ours score higher than one that simply lists everything.
 */
...
export function matchCommunityBuilds(
  ...
  for (const build of builds) {
    ...
    const { jaccard, shared } = overlap(ours, build.itemIds);
    ranked.push({
      build,
